...
主题贯穿：**注塑机控制系统**。每一关都遵循 `读指导书 → 实现 API → 跑通 CTest → 解锁下一关` 的闭环，前面写的数据结构会在后面的并发 / 网络 / 嵌入式实验里被反复复用。
...
```mermaid
flowchart TD
    L0["Lab0 环境与构建 CMake/CTest"] --> L1["Lab1 环形缓冲区"]
...
> 说明：Track A/B/C（Lab 1~11）的脚手架与指导书均已就绪。为保持"实现 → 测试 → 解锁"的学习闭环，顶层 `CMakeLists.txt` 默认只启用当前关，过一关就取消下一行 `add_subdirectory(...)` 的注释来解锁。Track D（STM32H7，Lab 12~16）会在搭好交叉编译工具链后单独给出。路线图可按你入职后了解到的真实技术栈随时调整。
