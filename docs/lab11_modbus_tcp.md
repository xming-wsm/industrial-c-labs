...
2. 实现三个常用功能码：0x03 读保持寄存器、0x06 写单个寄存器、0x10 写多个寄存器；
...
```
请求 PDU: [0x10][起始地址 hi,lo][数量 hi,lo][字节数=2*数量][值 hi,lo]...
应答 PDU: [0x10][起始地址 hi,lo][数量 hi,lo]
```

校验：数量须在 1..123，且字节数须 == 2*数量，否则异常 0x03；起始地址 + 数量 须 <= 寄存器总数，否则异常 0x02。
...
uint32_t mb_request_count(const modbus_slave_t *mb);    /* 累计产生应答的请求数 */
uint32_t mb_exception_count(const modbus_slave_t *mb);  /* 其中异常应答的次数 */
...
6. 每次产生应答（含异常应答）时 `request_count` +1，其中异常应答再让 `exception_count` +1；返回 -1 的畸形请求不计数。
...
6. 实现 FC 0x10（写多个寄存器），跑 `test_write_multiple` 及其异常用例。
7. 补齐异常分支（地址/数值/功能码）与请求计数，跑剩余用例。
...
全部实现后应看到 `==== summary: 11 run, 0 failed ====`。
